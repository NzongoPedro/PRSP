...
CREATE TABLE IF NOT EXISTS `bd_prsp`.`categoria_posto` (
  ...
  `categoriaPostoDesignacao` VARCHAR(45) NOT NULL DEFAULT 'Notário',
  `postos_idposto` INT NOT NULL,
  PRIMARY KEY (`idcategoria_posto`),
  INDEX `fk_categoria_posto_postos1_idx` (`postos_idposto` ASC),
  CONSTRAINT `fk_categoria_posto_postos1`
    FOREIGN KEY (`postos_idposto`)
    REFERENCES `bd_prsp`.`postos` (`idposto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bd_prsp`.`solicitacao_reserva` (
  ...
  INDEX `fk_solicitacao_reserva_comprovativo_reserva1_idx` (`idComprovativo` ASC),
  ...
  CONSTRAINT `fk_solicitacao_reserva_comprovativo_reserva1`
    FOREIGN KEY (`idComprovativo`)
    REFERENCES `bd_prsp`.`comprovativo_reserva` (`idcomprovativo_reserva`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
